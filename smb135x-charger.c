...
static ssize_t my_chg_iusb_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	unsigned long r;
	...
	printk(KERN_INFO "smb1359: my iusb store.");
	r = kstrtoul(buf, 0, &usb_curr);
	if (r) {
		pr_err("Invalid iusb value = %lu\n", usb_curr);
		return -EINVAL;
	...
	r = smb135x_masked_write(the_chip, CFG_C_REG,
				     USBIN_INPUT_MASK, i);
	if (r < 0) {
		dev_err(the_chip->dev,
			"Couldn't set USBIN Current = %d r = %d\n",
...
static ssize_t my_chg_iusb_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	int state;
	...
	printk(KERN_INFO "smb1359: my iusb show.");
	if (!the_chip) {
		pr_err("chip not valid\n");
		ret = -ENODEV;
		goto end;
...
static DEVICE_ATTR(my_chg_iusb, 0664,
		my_chg_iusb_show,
		my_chg_iusb_store);
				...
				const struct i2c_device_id *id)
{
	...
	rc = device_create_file(chip->dev,
			&dev_attr_my_chg_iusb);
	if (rc) {
		pr_err("couldn't create my_chg_iusb\n");
		goto unregister_dc_psy;
	}
